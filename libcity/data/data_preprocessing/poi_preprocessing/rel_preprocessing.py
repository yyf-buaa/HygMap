...
#计算两条路之间的dist
geo_df = pd.read_csv("/home/jjw/private/raw_data/sf/poimap_sf/poimap_sf.geo")
weight = []
points = []
dist_mtx = np.zeros([poi_num,poi_num])
for i in tqdm(range(len(geo_df))):
    points.append(Point(geo_df.loc[i,'X'],geo_df.loc[i,'Y']))
for i in tqdm(range(len(points))):
    for j in range(len(points)):
        dist_mtx[i][j] = points[i].distance(points[j])
np.save("/home/jjw/private/raw_data/sf/poimap_sf/poimap_sf_dist_mtx.npy",dist_mtx)
# for i in tqdm(range(len(geo_df))):
