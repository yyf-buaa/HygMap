...
class HyperMap(nn.Module):
    def __init__(self, config, data_feature):
        super(HyperMap, self).__init__()
        ...
        self.poi_drop_edge_rate_1 = config['poi_drop_edge_rate_1']
        self.poi_drop_edge_rate_2 = config['poi_drop_edge_rate_2']
        self.poi_drop_feature_rate_1 = config['poi_drop_feature_rate_1']
        self.poi_drop_feature_rate_2 = config['poi_drop_feature_rate_2']
        ...
        self.poi_emb_module = POIEmbedding(config,data_feature)
        # graph encoder
        self.intra_graph_encoder = HGT(hidden_channels=self.hid_dim, out_channels=self.hid_dim, num_heads=self.heads,
                                    ...
                                    node_types=['poi_node','road_node','region_node'], metadata=self.meta_data)
        self.inter_hyper_encoder = HyperEncoder(in_dim=self.hid_dim,edge_dim=self.hid_dim,node_dim=self.hid_dim)
        # projection head
        self.poi_fc1 = nn.Linear(self.hid_dim, self.num_proj_hidden)
        self.poi_fc2 = nn.Linear(self.num_proj_hidden, self.hid_dim)
    ...
    def encode_poi_road_region(self, data_poi_x, data_road_x, data_region_x, graph_edge_index,hyperedge_index, emb=True):
        if emb:
            poi_emb_init = self.poi_emb_module(data_poi_x)
        ...
        data_input = {'poi_node':poi_emb_init, 'road_node': road_emb_init, 'region_node': region_emb_init}
        # road_embedding, region_embedding = self.encoder(data_input, data_edge_index_dict)  # (N-region, hid)
        poi_embedding, road_embedding, region_embedding = self.intra_graph_encoder(data_input, graph_edge_index)
        ...
        hyperNode_embedding, hyperedge_embedding = self.inter_hyper_encoder(x=hyperNode_embedding,e=hyperedge_embedding,hyperedge_index=hyperedge_index)
    ...
    def forward(self, data):
        if self.intra_road2region:
            poi_emb_init ,road_emb_init, region_emb_init = self.encode_poi_road_region(data_poi_x=data['intra_graph']['poi_node'].x, data_road_x=data['intra_graph']['road_node'].x,data_region_x= data['intra_graph']['region_node'].x,
                                                                         graph_edge_index=data['intra_graph'].edge_index_dict,
                                                                         hyperedge_index=data['hyperedge_index'],emb=True)  # (N-road, hid), (N-region, hid)
        else:
            poi_emb_init = self.poi_emb_module(data['intra_graph']['poi_node'].x)
            road_emb_init = self.road_emb_module(data['intra_graph']['road_node'].x)
            region_emb_init = self.region_emb_module(data['intra_graph']['region_node'].x)
        poi_feature_drop_weights = self._get_data_fea_drop_weight(data['intra_graph'],poi_emb_init,'poi_node')
        road_feature_drop_weights = self._get_data_fea_drop_weight(data['intra_graph'], road_emb_init, 'road_node')
        region_feature_drop_weights = self._get_data_fea_drop_weight(data['intra_graph'], region_emb_init, 'region_node')

        poi_x_1 = drop_feature_weighted_2(poi_emb_init, poi_feature_drop_weights, self.poi_drop_feature_rate_1,
                                           threshold=0.7)
        poi_x_2 = drop_feature_weighted_2(poi_emb_init, poi_feature_drop_weights, self.poi_drop_feature_rate_2,
                                           threshold=0.7)
        ...
        poi_edge_index_geo_1, poi_edge_index_sem_1, poi_edge_index_mob_1 = \
            self._drop_edge(data['intra_graph'], 'poi_node', self.poi_drop_edge_rate_1)
        poi_edge_index_geo_2, poi_edge_index_sem_2, poi_edge_index_mob_2 = \
            self._drop_edge(data['intra_graph'], 'poi_node', self.poi_drop_edge_rate_2)

        road_edge_index_geo_1, road_edge_index_sem_1, road_edge_index_mob_1 = \
            self._drop_edge(data['intra_graph'], 'road_node', self.road_drop_edge_rate_1)
        road_edge_index_geo_2, road_edge_index_sem_2, road_edge_index_mob_2 = \
            self._drop_edge(data['intra_graph'], 'road_node', self.road_drop_edge_rate_2)

        region_edge_index_geo_1, region_edge_index_sem_1, region_edge_index_mob_1 = \
            self._drop_edge(data['intra_graph'], 'region_node', self.region_drop_edge_rate_1)
        region_edge_index_geo_2, region_edge_index_sem_2, region_edge_index_mob_2 = \
            self._drop_edge(data['intra_graph'], 'region_node', self.region_drop_edge_rate_2)
        edge_index_1 = {}
        edge_index_2 = {}
        if 'geo' in self.edge_types:
            edge_index_1[('poi_node', 'geo', 'poi_node')] = poi_edge_index_geo_1
            edge_index_1[('road_node', 'geo', 'road_node')] = road_edge_index_geo_1
            edge_index_1[('region_node', 'geo', 'region_node')] = region_edge_index_geo_1
            edge_index_2[('poi_node', 'geo', 'poi_node')] = poi_edge_index_geo_2
            edge_index_2[('road_node', 'geo', 'road_node')] = road_edge_index_geo_2
            edge_index_2[('region_node', 'geo', 'region_node')] = region_edge_index_geo_2
        if 'sem' in self.edge_types:
            edge_index_1[('poi_node', 'geo', 'poi_node')] = poi_edge_index_sem_1
            edge_index_1[('road_node', 'sem', 'road_node')] = road_edge_index_sem_1
            edge_index_1[('region_node', 'sem', 'region_node')] = region_edge_index_sem_1
            edge_index_2[('poi_node', 'geo', 'poi_node')] = poi_edge_index_sem_2
            edge_index_2[('road_node', 'sem', 'road_node')] = road_edge_index_sem_2
            edge_index_2[('region_node', 'sem', 'region_node')] = region_edge_index_sem_2
        if 'mob' in self.edge_types:
            edge_index_1[('poi_node', 'mob', 'poi_node')] = poi_edge_index_mob_1
            edge_index_1[('road_node', 'mob', 'road_node')] = road_edge_index_mob_1
            edge_index_1[('region_node', 'mob', 'region_node')] = region_edge_index_mob_1
            edge_index_2[('poi_node', 'mob', 'poi_node')] = poi_edge_index_mob_2
            edge_index_2[('road_node', 'mob', 'road_node')] = road_edge_index_mob_2
            edge_index_2[('region_node', 'mob', 'region_node')] = region_edge_index_mob_2

        if self.fea_agu and self.edge_agu:
            poi_z1, road_z1, region_z1 = self.forward_intra(data_poi_x= poi_x_1,data_road_x=road_x_1, data_region_x=region_x_1,graph_edge_index=edge_index_1,hyperedge_index=data['hyperedge_index'])
            poi_z2, road_z2, region_z2 = self.forward_intra(data_poi_x= poi_x_2,data_road_x=road_x_2, data_region_x=region_x_2,graph_edge_index=edge_index_2,hyperedge_index=data['hyperedge_index'])
        elif self.fea_agu and not self.edge_agu:
            poi_z1, road_z1, region_z1 = self.forward_intra(data_poi_x= poi_x_1,data_road_x=road_x_1, data_region_x=region_x_1,graph_edge_index=data['intra_graph'].edge_index_dict,hyperedge_index=data['hyperedge_index'])
            poi_z2, road_z2, region_z2 = self.forward_intra(data_poi_x= poi_x_2,data_road_x=road_x_2, data_region_x=region_x_2,graph_edge_index=data['intra_graph'].edge_index_dict,hyperedge_index=data['hyperedge_index'])
        elif not self.fea_agu and self.edge_agu:
            poi_z1, road_z1, region_z1 = self.forward_intra(data_poi_x=poi_emb_init, data_road_x=road_emb_init,
                                                            data_region_x=region_emb_init, graph_edge_index=edge_index_1,
                                                            hyperedge_index=data['hyperedge_index'])
            poi_z2, road_z2, region_z2 = self.forward_intra(data_poi_x=poi_emb_init, data_road_x=road_emb_init,
                                                            data_region_x=region_emb_init, graph_edge_index=edge_index_2,
                                                            hyperedge_index=data['hyperedge_index'])
        else:
            poi_z1, road_z1, region_z1 = self.forward_intra(data_poi_x=poi_emb_init, data_road_x=road_emb_init,
                                                            data_region_x=region_emb_init, graph_edge_index=data['intra_graph'].edge_index_dict,
                                                            hyperedge_index=data['hyperedge_index'])
            poi_z2, road_z2, region_z2 = self.forward_intra(data_poi_x=poi_emb_init, data_road_x=road_emb_init,
                                                            data_region_x=region_emb_init, graph_edge_index=data['intra_graph'].edge_index_dict,
                                                            hyperedge_index=data['hyperedge_index'])
        poi_z,road_z,region_z = self.encode_poi_road_region(data_poi_x=data['intra_graph']['poi_node'].x, data_road_x=data['intra_graph']['road_node'].x,data_region_x= data['intra_graph']['region_node'].x,
                                                                         graph_edge_index=data['intra_graph'].edge_index_dict,
                                                                         hyperedge_index=data['hyperedge_index'],emb=True)
        return poi_z1 , road_z1, region_z1, poi_z2 , road_z2, region_z2, poi_z, road_z, region_z
    ...
    def poi_projection(self, z: torch.Tensor):
        z = F.elu(self.poi_fc1(z))
        return self.poi_fc2(z)
