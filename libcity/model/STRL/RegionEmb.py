...
class RegionEmbedding(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.seq_cat_emb = nn.Linear(self.emb_dim * 4, self.hid_dim)

    def forward(self, batch_seq_cat):
        """
            batch_seq_cat: (B, len_seqs_cat_feas)
        """
        region_len_Shape_Leng_encode_emb = self.region_len_Shape_Leng_encode_emb(batch_seq_cat[:, 0])
        region_len_Shape_Area_encode_emb = self.region_len_Shape_Area_encode_emb(batch_seq_cat[:, 1])
        region_len_lon_encode_emb = self.region_len_lon_encode_emb(batch_seq_cat[:, 2])
        region_len_lat_encode_emb = self.region_len_lat_encode_emb(batch_seq_cat[:, 3])
        #region_len_Flag_MHI_P_emb = self.region_len_Flag_MHI_P_emb(batch_seq_cat[:, 0])

        cat_list = [ region_len_Shape_Leng_encode_emb,
