...
class UniGCNIIConv(MessagePassing):
    ...
    def forward(self, X, E, hyperedge_index, alpha, beta, X0, E0, degV, degE):
        ...
        N = X.shape[0]
        #wave 1
        Xe1 = self.propagate(hyperedge_index, x=X, norm=norm_n2e,
                            size=(num_nodes, num_edges))  # Node to edge: First pooling
        Xe1 = Xe1 * degE
        Xv1 = self.propagate(hyperedge_index.flip([0]), x=Xe1, norm=None,
        ...
        #wave 2
        Xv2= self.propagate(hyperedge_index.flip([0]), x=E, norm=None,
                            size=(num_edges, num_nodes))  # Edge to node

        Xv2 = Xv2 * degV
        Xe2 = self.propagate(hyperedge_index, x=Xv2, norm=norm_n2e,
                            size=(num_nodes, num_edges))  # Node to edge: First pooling
        Xe2 = Xe2 * degE
...
class HyperDecoder(nn.Module):
    ...
    def forward(self, x: Tensor, e:Tensor ,hyperedge_index: Tensor, num_nodes: int, num_edges: int):
        ...
        for i, conv in enumerate(self.convs[1:]):
            ...
            beta = math.log(lamda / (i + 1) + 1)
            if i == len(self.convs[1:]) - 1:
                x = conv(x, hyperedge_index, alpha, beta, x0, e0, degV, degE)
            else:
                x = torch.relu(conv(x, hyperedge_index, alpha, beta, x0, e0, degV, degE))
