    ...
    for fold_num, (train_idx, val_idx) in enumerate(kf.split(X, y)):
        ...
        if choice <100:
            ...
            if fold_num==0:
                ...
                after=len(X_train)
                print(f"after is {after}")
        X_train = torch.tensor(X_train).cuda().to(torch.float32)
        X_eval = torch.tensor(X_eval).cuda().to(torch.float32)
        y_train = torch.tensor(y_train).cuda().to(torch.float32)
        y_eval = torch.tensor(y_eval).cuda().to(torch.float32)
...
class RoadEvaluator():
    ...
    def __init__(self,config):
        ...
        self.road_embedding_path = config.get('path', None)
    ...
    def _valid_clf(self, emb):
        ...
        for i, j in enumerate(useful_label):
            relabel[j] = i
        useful_index = []
        self._logger.info(f'road emb shape = {emb.shape}, label shape = {label.shape}')
    ...
    def _valid_flow_using_bilinear(self, emb):
        od_flow = np.load(
                        './raw_data/{}/traj_road_test_od.npy'.format(
                self.dataset))
    ...
    def _valid_road_speed(self, road_emb):
        self._logger.warning('Evaluating Road Average Speed Prediction')
        speed_df = pd.read_csv('./raw_data/{}/avg_speed.csv'.format(self.dataset))
        road_emb = road_emb[speed_df['index'].values]
        speed = speed_df['speed'].values
        speed_mae, speed_rmse, speed_mape, speed_r2 = evaluation_reg(road_emb, speed, kfold=5, seed=self.seed, output_dim=self.output_dim)
        print(
            "Result of {} estimation in {}:".format('avg_speed', self.dataset))
        print(
            'MAE = {:6f}, RMSE = {:6f}'.format(speed_mae, speed_rmse))
        return speed_mae, speed_rmse

    def _valid_flow(self, emb):
        self._logger.warning(f'Evaluating road In-Flow Prediction')
        inflow = np.load('./raw_data/{}/traj_road_test_in_avg.npy'.format(self.dataset))
        ...
        self._logger.warning(f'Evaluating road Out-Flow Prediction')
        outflow = np.load('./raw_data/{}/traj_road_test_out_avg.npy'.format(self.dataset))
        ...
        r2 = (in_r2 + out_r2) / 2
        self._logger.info(f"Result of road flow estimation in {self.dataset}:")
        self._logger.info('MAE = {:6f}, RMSE = {:6f}, R2 = {:6f}, MAPE = {:6f}'.format(mae, rmse, r2, mape))
        return mae, rmse, r2, mape
...
args_dict = vars(args)
road_evaluator = RoadEvaluator(args_dict)
road_evaluator.evaluate()